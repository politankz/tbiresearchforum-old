...
//    dynModDictKeys = [[dynModDict allKeys] sortedArrayUsingSelector:@selector(compare:)];
    ...
    [self loadPostersInDict:allPosterDict];
    
//    moduleName = [dynModDict objectForKey:kModuleNameKey];
    
}

- (void)loadPostersInDict:(NSDictionary *)thisDictionary {
    int postersToLoad = [[thisDictionary allKeys] count];
    NSLog(@"Loading %d posters...",postersToLoad);
    
    int posterIndex = 0;
    
    for (NSString *thisKey in [thisDictionary allKeys]) {
        NSString *currentDictKey = [NSString stringWithFormat:@"poster%d",posterIndex+1];
...
//        for (NSDictionary *posterDict in thisDictionary) {
            ...
            [posterAuthorLasts addObject:[currentDictionary objectForKey:@"lasts"]];
...
//        }
    ...
    NSLog(@"Load poster progress: %d of %d posters loaded!",posterIndex,postersToLoad);
}
...
- (void)viewDidLoad
{
    ...
    NSString *currentPostersPlistFilename = @"tbirf13_abstracts_v1";
...
//    [cellImage setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%d.jpg", indexPath.row]]];
    ...
    [cellLasts setText:[posterAuthorLasts objectAtIndex:indexPath.row]];
